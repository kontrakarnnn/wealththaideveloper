     ...
     public function quotationcustomer($id)
      {
        ...
        $findcoor = User::where('id',$findcase->coordinate_user_block_id)->value('id');
        $findmatchidcoor= match_id::where('user_id',$findcoor)->value('id');
        $matchidcoor= match_id::find($findmatchidcoor);
        if($matchidcoor->public_name == NULL ||$matchidcoor->public_mobile == NULL||$matchidcoor->public_name == '' ||$matchidcoor->public_mobile == ''){
          $coorname ='พรพิมล';
          $coormobile ='0900956942' ;
        }
        else
        {
          $coorname = $matchidcoor->public_name;
          $coormobile = $matchidcoor->public_mobile;
        }
        ...
        $findofferact = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id',7)->orderBy('id','DESC')->get()->take(1);
        $countoffact = count($findofferact);
        $findofferact = $findofferact->chunk(4);
        $findoffertax = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id',8)->orderBy('id','DESC')->get()->take(1);
...
foreach($findoffertax as $chunk)
{
 ...
 $html .="</body>";
 $filename = "Quotation_".$customername;
 // include autoloader

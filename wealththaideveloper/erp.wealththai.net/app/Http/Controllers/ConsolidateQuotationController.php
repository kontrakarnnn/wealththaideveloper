<?php

namespace App\Http\Controllers;

use Illuminate\Http\Request;
use Illuminate\Support\Facades\DB;
use App\Structure;
use Illuminate\Support\Facades\Auth;
use App\Block;
use App\View;
use App\Asset_cat;
use App\ActionCategory;
use App\User;
use App\User_auth;
use App\Partner;
use App\Partner_block;
use App\CaseChannel;
use App\Person;
use App\Member_type;
use App\Country;
use App\Subdistrict;
use App\District;
use App\Province;
use App\Asset_type;
use App\match_member_id;
use App\Portfolio;
use App\Asset;
use App\CaseCategory;
use App\CaseType;
use App\CaseSubType;
use App\Cases;
use App\match_id;
use App\Procedures_To_Process;
use App\Process;
use App\Member_group;
use App\Family;
use App\Pid_group;
use App\Partner_group;
use App\CaseAuth;
use App\File;
use App\Asset_Attacht;
use App\Case_Attacht;
use App\Offer;
use App\Proposal;
use App\Case_proposal;
use App\Case_log;
use App\Casemiddledata;
use App\Casemiddledatatype;
use App\Offer_Attacht;
use App\Promotion;
use App\Case_condition;
use App\CaseAction;
use App\Stage;
use PDF;
use App\Http\Controllers\CaseCenterController;
use FPDF;
use Dompdf\Dompdf;
use Illuminate\Support\Facades\Hash;

use App\Http\Controllers\SidebarController;
class ConsolidateQuotationController extends Controller
{
    /**
     * Create a new controller instance.
     *
     * @return void
     */

    public function __construct()
    {
        $this->middleware('view');
    }
     public function consolidatequotation($id)
      {
        $day = date("d");
        $month = date("m");
        $year = date("Y")+543;
        $date = $day.'/'.$month.'/'.$year;
        $currentuserid = Auth::user()->id;
        $matchid = match_id::where('user_id',$currentuserid)->value('id');
        $matchid= match_id::find($matchid);
        $username = $matchid->public_name;
        $useremail = $matchid->public_email;
        $usermobile = $matchid->public_mobile;
        $findcase = Cases::with(['Person','Stage','Cases','Block','Partner_block','CaseType','CaseSubType','Asset','match_id','CaseStatus','coordiantor','CaseChannel'])->find($id);
        $customername = $findcase->person->name;
        $customerlastname = $findcase->person->lname;
        $findasset = Asset::find($findcase->referal_asset);
        $findproposal = Proposal::where('case_id',$id)->pluck('id')->toArray();
        $findoffer = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id','!=',7)->where('type_id','!=',8)->get()->sortby('ref_member_id');
        $countoff = count($findoffer);
        $findoffer = $findoffer->chunk(4);
        $findofferact = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id',7)->get();
        $countoffact = count($findofferact);
        $findofferact = $findofferact->chunk(4);
        $findoffertax = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id',8)->get();
        $findoffertax =  $findoffertax->chunk(4);
 	   $wealththailogo = 'data:image/png;base64,';
 		 $carinsurancelogo = 'data:image/png;base64,';
 		$html = "<style>
 @font-face {
   font-family: 'THSarabunNew';
   font-style: normal;
   font-weight: normal;
   src: url(http://wealththai.org/fonts/THSarabunNew.ttf) format('truetype');
 }
 @font-face {
   font-family: 'THSarabunNew-Bold';
   font-style: normal;
   font-weight: 700;
   src: url(http://wealththai.org/fonts/THSarabunNew Bold.ttf) format('truetype');
 }
 body{    font-family: 'THSarabunNew';
 }

 table {
   border-collapse: collapse;
   border-spacing:0.5px;
   width: 100%;
   font-size: 15.5px
 }
 .table
 {
   font-size:11px;
     border:solid 0.5px;
     text-align:center;
 }
 .columnshow10 {
   float:left;
   padding:10px;
   width: 10%;
   margin-top:-15px
 }
 .wealth
 {
   margin-top:-15px;
   max-width:100px
 }
 .carinsur
 {
   margin-top:-20px;
   max-width:100px
 }
 .footer {
     position: fixed;
             bottom: 45;
             width: 100%;

         }
         .header {
             position: fixed;
                     top: 0;
                     width: 100%;
                 }
         .page-break {
   page-break-after: always;
}
header { position: fixed;
        top: 0;
        width: 100%; }
 </style>";
 $html .= "<body>  <header>
     <div class='header'>

     <img class='wealth' src='{$wealththailogo}'  />
       <img class='carinsur' src='{$carinsurancelogo}' />

   <div style='width:450px;float:right;margin-top:-60px' >
     <p style='text-align:right;font-size:14px'>
       'ต่อประกันครั้งหน้า อย่าลืมคิดถึงเรา
  เพราะความพึงพอใจของคุณ คือเป้าหมายสูงสุดของเรา'
     </p>
   </div>
   <br />
   <br />
   <br />



 <p style='text-align:center;font-size:18px;margin-top:-70px'>ใบเสนอราคา/Quotation</p>
 <div style='text-align:right'>
   <table style='width:250px;border:none;margin-left:350px;margin-top:-80px'>
     <tr>
     <td>วันที่</td>
     <td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>{$date}</td>
   </tr>
   <tr style='padding-bottom: 0;'>
     <td>เสนอราคาโดย</td>
     <td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>{$username}  ({$usermobile})</td>
   </tr>
     <tr>

     <td>E-mail</td>
     <td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>&nbsp;</td><td>{$useremail}</td>
   </tr>
   </table>
 </div>
 <div style='text-align:left'>
   <table style='width:250px;border:none;margin-top:-70px'>
     <tr>
     <td>เรียนคุณ</td>
     <td>&nbsp;</td><td> {$customername} {$customerlastname}</td>
   </tr>
   <tr style='margin-top:-160px'>
     <td>เรื่อง</td>
     <td>&nbsp;</td><td>ข้อเสนอราคากรมธรรมประกันภัย</td>
   </tr>
   </table>
 </div>
 <br/>
 <table style='margin-top:-20px' class='table'>
   <tr>
     <td width:10% class='table'style='background-color:#dedee3;text-align:left;'>รหัสรถยนต์</td>
     <td  class='table'style='background-color:#dedee3'>ยี่ห้อ</td>
     <td class='table' style='background-color:#dedee3'>รุ่น</td>
     <td  class='table'style='background-color:#dedee3;'>เลขทะเบียน</td>
     <td  class='table'style='background-color:#dedee3'>ปี/รุ่น</td>
     <td class='table'style='background-color:#dedee3;text-align:left;'>รุ่นย่อย</td>
     <td  class='table' style='background-color:#dedee3'>มูลค่าอุปกรณ์เสริม</td>
   </tr>
   <tr>
     <td class='table'>{$findasset->ref_info7}</td>
     <td  class='table'>{$findasset->ref_info3}</td>
     <td  class='table'>{$findasset->ref_info4}</td>
     <td  style=''class='table'>{$findasset->ref_name}</td>
     <td class='table'>{$findasset->ref_info5}</td>
     <td  class='table'>{$findasset->ref_info6}</td>
     <td  class='table'>{$findasset->ref_info13}</td>
   </tr>
   <tr>
     <td class='table'style='background-color:#dedee3;text-align:left;'>รายการอุปกรณ์เสริม</td>
     <td colspan='6' class='table'></td>
   </tr>

 </table>

 </div>
 </header>

 ";

foreach ($findoffer as $index =>$chunk)
{
  $colspan = count($chunk)+1;
  $html .="<table style='margin-top:160px' class='table'>

    <tr>

        <td  colspan='{$colspan}' style='background-color:#7EC1D5'>
         กรมธรรมภาคสมัครใจ
      </td>
    </tr>";

      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ชื่อบริษัท พันธมิตร</td>";
      foreach($chunk as $findof)
      {
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $html .="<td class='table'>{$partnerblockname}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>บริษัทประกัน</td>";
      foreach($chunk as $findof)
      {
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $html .="<td class='table'>{$findof->Person->name}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ประเภทการซ่อม</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->OfferType->name}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ชนิดกรมธรรม์</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->OfferType->name}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ผู้ขับขี่</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value17}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td  colspan='{$colspan}' style='background-color:white'>ความเสียหายต่อภายนอก</td></tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ต่อคน</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value1}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ต่อครั้ง</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value2}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ทรัพย์สินต่อครั้ง</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value3}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ชดเชยความผิดส่วนแรก</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value4}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td  colspan='{$colspan}' style='background-color:white'>ทุนประกันรถยนต์</td></tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ความเสียหาย</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value3}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ไฟไหม้/สูญหาย</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value7}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>น้ำท่วม</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value14}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td  colspan='{$colspan}' style='background-color:white'>ความคุ้มครองต่อบุคคล</td></tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>จำนวนคน</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value19}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>อุบัติเหตุส่วนบุคคล</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value8}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ค่ารักษาพยาบาล</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value12}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ค่าประกันตัว</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value13}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td  colspan='{$colspan}' style='background-color:white'>ค่าเบี้ยประกัน</td></tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>เบียสุทธิ</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_payment_value1}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>เบียอากรแสตมป์สุทธิ</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_payment_value2}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ภาษีมูลค่าเพิ่ม</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_payment_value3}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ยอดสุทธิ</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_payment_value4}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ส่วนลดพิเศษ</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_payment_value15}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ราคาจ่ายจริง (เฉพาะชำระเงินสด)</td>";
      foreach($chunk as $findof)
      {
        $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
        $resultact = $offeract4- $offeract15;
        $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
        $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
        $resulttax = $offertax4- $offertax15;
        $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
        $allresult = ($result+$resultact)+$resulttax;
        $result2 = $result-$findof->offer_payment_value5;
        $result3 = $result2-$findof->offer_payment_value19;
        $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
        $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
        $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
        $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
        $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
        $conclu = $result-$allresultpaytax-$allresultucom;

        $html .="<td class='table'>{$result}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>หมายเหตุ</td>";
      foreach($chunk as $findof)
      {
        $html .="<td class='table'>{$findof->offer_value18}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>พรบ หน้าตั๋ว</td>";
      foreach($chunk as $findof)
      {
        $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
        $resultact = $offeract4- $offeract15;
        $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
        $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
        $resulttax = $offertax4- $offertax15;
        $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
        $allresult = ($result+$resultact)+$resulttax;
        $result2 = $result-$findof->offer_payment_value5;
        $result3 = $result2-$findof->offer_payment_value19;
        $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
        $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
        $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
        $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
        $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
        $conclu = $result-$allresultpaytax-$allresultucom;

        $html .="<td class='table'>{$offeract}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ราคาจ่ายจริง (พรบ/เฉพาะเงินสด)</td>";
      foreach($chunk as $findof)
      {
        $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
        $resultact = $offeract4- $offeract15;
        $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
        $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
        $resulttax = $offertax4- $offertax15;
        $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
        $allresult = ($result+$resultact)+$resulttax;
        $result2 = $result-$findof->offer_payment_value5;
        $result3 = $result2-$findof->offer_payment_value19;
        $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
        $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
        $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
        $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
        $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
        $conclu = $result-$allresultpaytax-$allresultucom;

        $html .="<td class='table'>{$resultact}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ราคาชำระค่าภาษี</td>";
      foreach($chunk as $findof)
      {
        $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
        $resultact = $offeract4- $offeract15;
        $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
        $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
        $resulttax = $offertax4- $offertax15;
        $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
        $allresult = ($result+$resultact)+$resulttax;
        $result2 = $result-$findof->offer_payment_value5;
        $result3 = $result2-$findof->offer_payment_value19;
        $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
        $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
        $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
        $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
        $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
        $conclu = $result-$allresultpaytax-$allresultucom;

        $html .="<td class='table'>{$resulttax}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>เบี้ยที่ต้องชำระรวม</td>";
      foreach($chunk as $findof)
      {
        $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
        $resultact = $offeract4- $offeract15;
        $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
        $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
        $resulttax = $offertax4- $offertax15;
        $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
        $allresult = ($result+$resultact)+$resulttax;
        $result2 = $result-$findof->offer_payment_value5;
        $result3 = $result2-$findof->offer_payment_value19;
        $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
        $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
        $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
        $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
        $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
        $conclu = $result-$allresultpaytax-$allresultucom;

        $html .="<td class='table'>{$allresult}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ภาษีหัก ณ ที่จ่าย (รวม)</td>";
      foreach($chunk as $findof)
      {
        $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
        $resultact = $offeract4- $offeract15;
        $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
        $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
        $resulttax = $offertax4- $offertax15;
        $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
        $allresult = ($result+$resultact)+$resulttax;
        $result2 = $result-$findof->offer_payment_value5;
        $result3 = $result2-$findof->offer_payment_value19;
        $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
        $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
        $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
        $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
        $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
        $conclu = $result-$allresultpaytax-$allresultucom;

        $html .="<td class='table'>{$allresultpaytax}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ค่าคอมมิชชั่นผู้ให้บริการ(รวม)</td>";
      foreach($chunk as $findof)
      {
        $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
        $resultact = $offeract4- $offeract15;
        $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
        $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
        $resulttax = $offertax4- $offertax15;
        $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
        $allresult = ($result+$resultact)+$resulttax;
        $result2 = $result-$findof->offer_payment_value5;
        $result3 = $result2-$findof->offer_payment_value19;
        $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
        $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
        $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
        $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
        $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
        $conclu = $result-$allresultpaytax-$allresultucom;

        $html .="<td class='table'>{$allresultucom}</td>";
      }
      $html .="</tr>";
      $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>เบี้ยที่ผู้ให้บริการต้องชำระ(รวม)</td>";
      foreach($chunk as $findof)
      {
        $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
        $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
        $resultact = $offeract4- $offeract15;
        $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
        $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
        $resulttax = $offertax4- $offertax15;
        $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
        $allresult = ($result+$resultact)+$resulttax;
        $result2 = $result-$findof->offer_payment_value5;
        $result3 = $result2-$findof->offer_payment_value19;
        $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
        $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
        $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
        $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
        $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
        $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
        $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
        $conclu = $result-$allresultpaytax-$allresultucom;
        $html .="<td class='table'>{$conclu}</td>";
      }
      $html .="</tr>";
      $html .="";
    $html.="</table> <div class='page-break'></div>";

}

  $findofferrefmem = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id','!=',7)->where('type_id','!=',8)->pluck('ref_member_id')->toArray();
  $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id',7)->get();
  $refmemidact = array();
  foreach($offeract as $of)
  {
    if(!in_array($of->ref_member_id,$findofferrefmem))
    {
       array_push($refmemidact,$of->ref_member_id);
    }
    else
    {
    }
  }
  if(count($findofferact) > 0 && $refmemidact != "" )
  {
    $findofferact = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->whereIn('ref_member_id',$refmemidact)->where('interest',1)->where('type_id',7)->get();
    $findofferact = $findofferact->chunk(4);
    foreach ($findofferact as $index =>$chunk)
    {
      $colspan = count($chunk)+1;
      $html .="<table style='margin-top:160px' class='table'>

        <tr>

            <td  colspan='{$colspan}' style='background-color:#7EC1D5'>
             <span style='color:red'>***บริษัทที่ไม่ได้เสนอเบี้ยกรมธรรม์***</span>
          </td>
        </tr>";

          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ชื่อบริษัท พันธมิตร</td>";
          foreach($chunk as $findof)
          {
            $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
            $html .="<td class='table'>{$partnerblockname}</td>";
          }
          $html .="</tr>";
          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>บริษัทประกัน</td>";
          foreach($chunk as $findof)
          {
            $html .="<td class='table'>{$findof->Person->name} </td>";
          }
          $html .="</tr>";
          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>พรบ หน้าตั๋ว</td>";
          foreach($chunk as $findof)
          {
            $html .="<td class='table'>{$findof->offer_payment_value4}</td>";
          }
          $html .="</tr>";
          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ราคาจ่ายจริง (พรบ/เฉพาะเงินสด)</td>";
          foreach($chunk as $findof)
          {
            $resultact = $findof->offer_payment_value4- $findof->offer_payment_value15;
            $html .="<td class='table'>{$resultact}</td>";
          }
          $html .="</tr>";
          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ราคาชำระค่าภาษี</td>";
          foreach($chunk as $findof)
          {
            $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
            $resultact = $offeract4- $offeract15;
            $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
            $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
            $resulttax = $offertax4- $offertax15;
            $html .="<td class='table'>{$resulttax}</td>";
          }
          $html .="</tr>";
          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>เบี้ยที่ต้องชำระรวม</td>";
          foreach($chunk as $findof)
          {
            $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
            $resultact = $offeract4- $offeract15;
            $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
            $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
            $resulttax = $offertax4- $offertax15;
            $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
            $allresult = ($result+$resulttax);
            $html .="<td class='table'>{$allresult}</td>";
          }
          $html .="</tr>";
          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ภาษีหัก ณ ที่จ่าย (รวม)</td>";
          foreach($chunk as $findof)
          {
            $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
            $resultact = $offeract4- $offeract15;
            $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
            $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
            $resulttax = $offertax4- $offertax15;
            $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
            $allresult = ($result+$resultact)+$resulttax;
            $result2 = $result-$findof->offer_payment_value5;
            $result3 = $result2-$findof->offer_payment_value19;
            $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
            $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
            $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
            $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
            $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
            $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
            $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
            $conclu = $result-$allresultpaytax-$allresultucom;

            $html .="<td class='table'>{$allresultpaytax}</td>";
          }
          $html .="</tr>";
          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ค่าคอมมิชชั่นผู้ให้บริการ(รวม)</td>";
          foreach($chunk as $findof)
          {
            $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
            $resultact = $offeract4- $offeract15;
            $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
            $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
            $resulttax = $offertax4- $offertax15;
            $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
            $allresult = ($result+$resultact)+$resulttax;
            $result2 = $result-$findof->offer_payment_value5;
            $result3 = $result2-$findof->offer_payment_value19;
            $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
            $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
            $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
            $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
            $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
            $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
            $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
            $conclu = $result-$allresultpaytax-$allresultucom;

            $html .="<td class='table'>{$allresultucom}</td>";
          }
          $html .="</tr>";
          $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>เบี้ยที่ผู้ให้บริการต้องชำระ(รวม)</td>";
          foreach($chunk as $findof)
          {
            $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
            $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
            $resultact = $offeract4- $offeract15;
            $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
            $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
            $resulttax = $offertax4- $offertax15;
            $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
            $allresult = ($result+$resultact)+$resulttax;
            $result2 = $result-$findof->offer_payment_value5;
            $result3 = $result2-$findof->offer_payment_value19;
            $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
            $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
            $allresultpaytax = ($offeract5+$offertax5)+$findof->offer_payment_value5;
            $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
            $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
            $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
            $allresultucom = ($offeract19+$offertax19)+$findof->offer_payment_value19;
            $conclu = $result-$allresultpaytax-$allresultucom;
            $html .="<td class='table'>{$conclu}</td>";
          }
          $html .="</tr>";
          $html .="";
        $html.="</table> <div class='page-break'></div>";
      }


}
$findofferrefmem = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id','!=',7)->where('type_id','!=',8)->pluck('ref_member_id')->toArray();
$findofferactrefmem = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id',7)->pluck('ref_member_id')->toArray();
$refmem = array_merge($findofferrefmem,$findofferactrefmem);
$offertax = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->where('interest',1)->where('type_id',8)->get();
$refmemid = array();

foreach($offertax as $of)
{
  if(!in_array($of->ref_member_id,$refmem))
  {
     array_push($refmemid,$of->ref_member_id);
  }
  else
  {
  }
}

if(count($findoffertax) > 0 && $refmemid != "" )
{
  $findoffertax = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->whereIn('proposal_id',$findproposal)->whereIn('ref_member_id',$refmemid)->where('interest',1)->where('type_id',8)->get();
  $findoffertax = $findoffertax->chunk(4);
  foreach ($findoffertax as $index =>$chunk)
  {
    $colspan = count($chunk)+1;
    $html .="<table style='margin-top:160px' class='table'>

      <tr>
          <td  colspan='{$colspan}' style='background-color:#7EC1D5'>
           <span style='color:red'>***บริษัทนี้ไม่ได้เสนอเบี้ยกรมธรรม์และพรบ***</span>
        </td>
      </tr>";

        $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ชื่อบริษัท พันธมิตร</td>";
        foreach($chunk as $findof)
        {
          $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
          $html .="<td class='table'>{$partnerblockname}</td>";
        }
        $html .="</tr>";
        $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>บริษัทประกัน</td>";
        foreach($chunk as $findof)
        {
          $html .="<td class='table'>{$findof->Person->name} </td>";
        }
        $html .="</tr>";
        $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ราคาชำระค่าภาษี</td>";
        foreach($chunk as $findof)
        {
          $resulttax =  $findof->offer_payment_value4- $findof->offer_payment_value15;
          $html .="<td class='table'>{$resulttax}</td>";
        }
        $html .="</tr>";
        $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>เบี้ยที่ต้องชำระรวม</td>";
        foreach($chunk as $findof)
        {
          $resulttax =  $findof->offer_payment_value4 - $findof->offer_payment_value15;
          $html .="<td class='table'>{$resulttax}</td>";
        }
        $html .="</tr>";
        $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ภาษีหัก ณ ที่จ่าย (รวม)</td>";
        foreach($chunk as $findof)
        {
          $html .="<td class='table'>{$findof->offer_payment_value5}</td>";
        }
        $html .="</tr>";
        $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>ค่าคอมมิชชั่นผู้ให้บริการ(รวม)</td>";
        foreach($chunk as $findof)
        {
          $html .="<td class='table'>{$findof->offer_payment_value19}</td>";
        }
        $html .="</tr>";
        $html .="<tr><td class='table'style='background-color:#dedee3;text-align:left; '>เบี้ยที่ผู้ให้บริการต้องชำระ(รวม)</td>";
        foreach($chunk as $findof)
        {
          $offeract = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
          $offeract4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value4');
          $offeract15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value15');
          $resultact = $offeract4- $offeract15;
          $offertax4 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value4');
          $offertax15 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value15');
          $resulttax = $offertax4- $offertax15;
          $result = $findof->offer_payment_value4- $findof->offer_payment_value15;
          $allresult = ($result+$resultact)+$resulttax;
          $result2 = $result-$findof->offer_payment_value5;
          $result3 = $result2-$findof->offer_payment_value19;
          $offeract5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value5');
          $offertax5 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value5');
          $allresultpaytax = $findof->offer_payment_value5;
          $partnerblockname = Partner_block::where('id',$findof->proposal->partner_block)->value('name');
          $offeract19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',7)->value('offer_payment_value19');
          $offertax19 = Offer::with(['promotion','campaign','OfferType','match_id','Person','branch','proposal'])->where('ref_member_id',$findof->ref_member_id)->where('interest',1)->where('type_id',8)->value('offer_payment_value19');
          $allresultucom = $findof->offer_payment_value19;
          $conclu = $result-$allresultpaytax-$allresultucom;
          $html .="<td class='table'>{$conclu}</td>";
        }
        $html .="</tr>";
        $html .="";
      $html.="</table> <div class='page-break'></div>";
    }
}
 $filename = "Consolidate_quotation(user)_".$customername;
 // include autoloader
 // reference the Dompdf namespace
 // instantiate and use the dompdf class
 $dompdf = new Dompdf();
 $dompdf->loadHtml($html);
 // (Optional) Setup the paper size and orientation
 $dompdf->setPaper('A4', 'protait');
 // Render the HTML as PDF
 $dompdf->render();
 // Output the generated PDF to Browser
 return $dompdf->stream($filename);
      }
}

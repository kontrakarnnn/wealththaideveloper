...
export default class UserRanking extends Component {
  ...
  Exportpdf()
  {
    ...
    //  dataRow.push([ 'No.', 'Structure Name', 'Block Name', 'Belong To', 'User Name', 'Block Commission', '% Income'] )
      ...
      dataRow.push(sourceRow.under_block_name);
      dataRow.push(this.showuserinblockpdf(sourceRow));
      dataRow.push(sourceRow.total_premium.toFixed(2));
    ...
    let header = "User Ranking Report"
...
pdfMake.createPdf(docDefinition).open()
  ...
  Searchclick()
  {
    ...
    axios.post('/wealththaiinsurance/report/filteruser',{
      fromdate:this.state.fromdateday+'/'+this.state.fromdatemonth+'/'+this.state.fromdateyear,
      todate:this.state.todateday+'/'+this.state.todatemonth+'/'+this.state.todateyear,
      blockid:this.state.blockid,
      underblock:this.state.underblock,
    ...
    axios.post('/wealththaiinsurance/report/userauth',{
     ...
     let sumall;
     let result = [];
     let sum = this.state.cases.map(data =>result.push(data.total_premium));
    ...
    showcasecominblock(data)
    {
     ...
     let sumall;
     let result = [];
     let sum= this.state.blockcommission.map(data =>result.push(data.total_premium));
     if(result.length>0)
     {
  ...
  tableshow()
  {
    ...
    else if (this.state.tableflag == 1)
    {
      return <div> <ReactHTMLTableToExcel
          ...
          <td>{data.name}</td>
          <td>{data.under_block_name}</td>
          {this.showuserinblock(data)}
          <td>{data.total_premium.toFixed(2)}</td>
...
if (document.getElementById('userranking')) {
    ReactDOM.render(<UserRanking />, document.getElementById('userranking'));
}

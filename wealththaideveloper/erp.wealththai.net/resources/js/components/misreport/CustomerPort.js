...
export default class CustomerPort extends Component {
  ...
  {
    var bodyData = [];
    bodyData.push([ {text:'No.',fillColor:'silver'}, {text:'Customer Name',fillColor:'silver'}, {text:'Block Name',fillColor:'silver'}, {text:'Portfolio Id',fillColor:'silver'}, {text:'Portfolio Name',fillColor:'silver'}, {text:'Premium',fillColor:'silver'}, {text:'% Income',fillColor:'silver'}]);
    this.state.customer.forEach((sourceRow,index) => {
      var dataRow = [];
      dataRow.push(++index);
      dataRow.push(sourceRow.name);
      dataRow.push(sourceRow.blockportname);
      dataRow.push(sourceRow.port_id);
      dataRow.push(sourceRow.port_name);
      dataRow.push(sourceRow.total_premium);
      dataRow.push(this.showcasecominblockpdf(sourceRow));
      bodyData.push(dataRow)
    });
    bodyData.push([ '', {text:'Total Number of Block',fillColor:'green'}, this.state.blockfilter.length,{text:'Total Number Of USER	',fillColor:'orange'} ,this.state.userauth.length, this.state.allblockcommission, '100%']);
    let header = "Customer Portfolio Ranking(Premium)"
...
pdfMake.createPdf(docDefinition).open()
  ...
  Searchclick()
  {
    ...
    axios.post('/wealththaiinsurance/report/filterport',{
      fromdate:this.state.fromdateday+'/'+this.state.fromdatemonth+'/'+this.state.fromdateyear,
      todate:this.state.todateday+'/'+this.state.todatemonth+'/'+this.state.todateyear,
      blockid:this.state.blockid,
      underblock:this.state.underblock,
  ...
  tableshow()
  {
      ...
      return <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
          ...
          <th style={{width:'10px'}}  tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">No. </th>
            ...
            <th   tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Block Name </th>
              <th   tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Portfolio Id</th>
              ...
              <th   tabindex="0" aria-controls="example2" rowspan="1" colspan="1" aria-label="port: activate to sort column ascending">Net Premium</th>
    ...
    else if (this.state.tableflag == 1)
    {
      return <div> <ReactHTMLTableToExcel
                    ...
                    table="table-to-xls"
                    filename="customer_portfolio_report"
                    sheet="tablexls"
          ...
          <tfoot>
          <tr class="pagebreak">
          <th colspan="2" style={{backgroundColor:'green'}}> Total Customer	</th>
    ...
    else if(this.state.tableflag == 2)
    {
      return <table id="example2" class="table table-bordered table-hover dataTable" role="grid" aria-describedby="example2_info">
          ...
          <tbody>
          <tr>
          <td colspan="7" style={{textAlign:'center',color:'red',fontSize:'18px'}}><b>ไม่พบข้อมูล</b></td>
          </tr>
          </tbody>
          <tfoot>
          <tr>
...
if (document.getElementById('customerport')) {
    ReactDOM.render(<CustomerPort />, document.getElementById('customerport'));
}

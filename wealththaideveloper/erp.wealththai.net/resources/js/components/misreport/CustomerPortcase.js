...
export default class CustomerPortcase extends Component {
  ...
  {
    var bodyData = [];
    bodyData.push([ {text:'No.',fillColor:'silver'}, {text:'Portfolio Id',fillColor:'silver'}, {text:'Portfolio Name',fillColor:'silver'}, {text:'Block Name ',fillColor:'silver'}, {text:'Customer Name ',fillColor:'silver'}, {text:'Total Cases',fillColor:'silver'}, {text:'% Cases',fillColor:'silver'}]);
    this.state.customer.forEach((sourceRow,index) => {
      var dataRow = [];
      dataRow.push(++index);
      dataRow.push(sourceRow.port_id);
      dataRow.push(sourceRow.port_name);
      dataRow.push(sourceRow.block_port_name);
    ...
    let header = "Customer Portfolio Ranking Report(Total Cases)"
...
pdfMake.createPdf(docDefinition).open()
  ...
  Searchclick()
  {
    ...
    axios.post('/wealththaiinsurance/report/filterportcase',{
      fromdate:this.state.fromdateday+'/'+this.state.fromdatemonth+'/'+this.state.fromdateyear,
      todate:this.state.todateday+'/'+this.state.todatemonth+'/'+this.state.todateyear,
      blockid:this.state.blockid,
      underblock:this.state.underblock,
    ...
    axios.post('/wealththaiinsurance/report/userauth',{
     ...
     let sumall;
     let result = [];
     let sum = this.state.customer.map(data =>result.push(data.total_case));
  ...
  tableshow()
  {
    ...
    else if (this.state.tableflag == 1)
    {
      return <div> <ReactHTMLTableToExcel
                    ...
                    table="table-to-xls"
                    filename="customer_port_ranking_cases"
                    sheet="tablexls"
...
if (document.getElementById('customerportcase')) {
    ReactDOM.render(<CustomerPortcase />, document.getElementById('customerportcase'));
}

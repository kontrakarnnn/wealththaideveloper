...
export default class CustomerRankingcase extends Component {
  ...
  {
    var bodyData = [];
    bodyData.push([ {text:'No.',fillColor:'silver'}, {text:'Customer Name',fillColor:'silver'}, {text:'Total Cases',fillColor:'silver'}, {text:'% Cases',fillColor:'silver'}]);
    ...
    this.state.customer.forEach((sourceRow,index) => {
      var dataRow = [];
    //  dataRow.push([ 'No.', 'Structure Name', 'Block Name', 'Belong To', 'User Name', 'Block Commission', '% Cases'] )
      dataRow.push(++index);
    ...
    bodyData.push([ {text:'Total Number of Customer',fillColor:'green'}, this.state.blockfilter.length ,this.state.allblockcommission, '100%']);
    let header = "Customer Ranking Report(Total Cases)"
...
pdfMake.createPdf(docDefinition).open()
  ...
  Searchclick()
  {
    ...
    axios.post('/wealththaiinsurance/report/filtercustomercase',{
      fromdate:this.state.fromdateday+'/'+this.state.fromdatemonth+'/'+this.state.fromdateyear,
      todate:this.state.todateday+'/'+this.state.todatemonth+'/'+this.state.todateyear,
      blockid:this.state.blockid,
      underblock:this.state.underblock,
  ...
  tableshow()
  {
    ...
    else if (this.state.tableflag == 1)
    {
      return <div> <ReactHTMLTableToExcel
                    ...
                    table="table-to-xls"
                    filename="customer_ranking_report_case"
                    sheet="tablexls"
...
if (document.getElementById('customerrankingcase')) {
    ReactDOM.render(<CustomerRankingcase />, document.getElementById('customerrankingcase'));
}

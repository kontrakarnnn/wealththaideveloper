...
export default class CustomerPerformance extends Component {
  ...
  {
    var bodyData = [];
    bodyData.push([ {text:'No.',fillColor:'silver'}, {text:'Structure Name',fillColor:'silver'}, {text:'Block Name',fillColor:'silver'}, {text:'Belong To',fillColor:'silver'}, {text:'User Name',fillColor:'silver'}, {text:'Customer Name',fillColor:'silver'}, {text:'Premium',fillColor:'silver'}, {text:'% Income',fillColor:'silver'}]);
    this.state.blockfilter.forEach((sourceRow,index) => {
      var dataRow = [];
    //  dataRow.push([ 'No.', 'Structure Name', 'Block Name', 'Belong To', 'User Name', 'Premium', '% Income'] )
      dataRow.push(++index);
      dataRow.push(sourceRow.structure.name);
      dataRow.push(sourceRow.name);
      dataRow.push(this.showunderblockpdf(sourceRow));
      dataRow.push(this.showuserinblockpdf(sourceRow));
      dataRow.push(this.showcustomerpdf(sourceRow));
      dataRow.push(this.showcasecominblockpdf(sourceRow));
      dataRow.push(this.showcaseincomeinblockpdf(sourceRow));
      bodyData.push(dataRow)
    });
    bodyData.push([ '', {text:'Total Block',fillColor:'green'}, this.state.blockfilter.length,{text:'Total User	',fillColor:'orange'} ,this.state.userauth.length,'', this.state.allblockcommission, '100%']);
    let header = "Customer Performance Report"
...
pdfMake.createPdf(docDefinition).open()
  ...
  Searchclick()
  {
    ...
    axios.post('/wealththaiinsurance/report/userauth',{
     ...
     let sumall;
     let result = [];
     let sum = this.state.cases.map(data =>result.push(data.offer.offer_payment_value1));
     console.log("Result"+result.length);
     if(result.length>0)
     {

        this.state.allblockcommission =  result.reduce((result2,number)=> Number(result2)+Number(number));
        this.state.allblockcommission = this.state.allblockcommission.toFixed(2);
     }
     else
     {
    ...
    showcaseincomeinblockpdf(sourceRow)
    {
     ...
     let ans;
     let result = [];
     let sum= income.map(data =>result.push(data.offer.offer_payment_value1));
     if(result.length>0)
     {
    ...
    showcustomer(data)
    {
      ...
      this.state.customer = this.state.cases.filter((getcases) => {
    //    console.log("Case"+getcases.service_user_block_id == data.id);
       return getcases.cases.service_user_block_id == data.id
     })
     const uniqueTags = [];
     this.state.customer.map(img => {
    if (uniqueTags.indexOf(img.cases.person.name) === -1) {
        uniqueTags.push(img.cases.person.name)
    }
  });
     return <td>{uniqueTags.map((name,index) =><p>{++index}.{name} </p>)}</td>
    }

    showcustomerpdf(data)
    {
      //console.log("hey"+data.id);
      this.state.customer = this.state.cases.filter((getcases) => {
    //    console.log("Case"+getcases.service_user_block_id == data.id);
    ...
    if (uniqueTags.indexOf(img.cases.person.name) === -1) {
     ...
     return uniqueTags.map((name,index) => name )
  ...
  tableshow()
  {
    ...
    else if (this.state.tableflag == 1)
    {
      return <div> <ReactHTMLTableToExcel
                    ...
                    table="table-to-xls"
                    filename="customer_perfomance_report"
                    sheet="tablexls"
...
if (document.getElementById('customerformance')) {
    ReactDOM.render(<CustomerPerformance />, document.getElementById('customerformance'));
}

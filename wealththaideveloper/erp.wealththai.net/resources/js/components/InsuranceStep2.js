...
handleChangeCasetype(e){
  ...
  let coorfil = this.state.coordinator.filter((coordinator) => {
    ...
    this.setState({
      ...
      username2:[userfil.name],
      partnername2:[partnerfil.name],
  ...
  renderComponentByState(){
      if(this.state.addmemberflag === 0) {
        let options = this.state.member.map(function (member) {
          ...
          <td >
          <a style={{float:'left'}} onClick={this.openaddmember} >เพิ่มผู้เอาประกัน</a>
          </td>
          ...
          if(this.state.membertypeid == 2)
          {
            ...
            <th >ผู้แนะนำ</th>
            <td ><div style={{width: '190px'}}>
            <select style={{width: '190px'}} onChange={(e) => this.setState({ belongtopidmember: e.target.value })} class="form-control" name="belongtopidmember">
            <option value ="">โปรดเลือก</option>
            {
              this.state.memberpid.map(
                data =>
